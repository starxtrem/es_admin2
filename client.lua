...
RegisterNetEvent('es_admin:checkC')
AddEventHandler('es_admin:checkC', function(names, ids)
    idPlayers = ids
    namePlayers = names
...
Citizen.CreateThread(function()
	while true do
		...
		if (IsControlJustPressed(1, 212) and IsControlJustPressed(1, 213)) then
			if group ~= "user" then
				TriggerServerEvent('es_admin:checkS')
				Wait(1000)
...
RegisterNetEvent('es_admin:Admin_BringC')
AddEventHandler('es_admin:Admin_BringC', function(plyCoords)
...
AddEventHandler('es_admin:quick', function(t, target)
	...
	if t == "bring" then 
		TriggerEvent('es_admin:Admin_BringC' ,GetPlayerFromServerId(target))
	end
	if t == "crash" then 
...
--Enable if you dont have personal menu for noclip
