			...
			TriggerEvent('es:canGroupTarget', user.getGroup(), groupsRequired[type], function(available)
				TriggerEvent('es:canGroupTarget', user.getGroup(), target.getGroup(), function(canTarget)
					if canTarget and available then
						...
						if type == "bring" then --TriggerClientEvent('es_admin:quick', id, type, Source) 
							TriggerClientEvent('es_admin:teleportUser', target.get('source'), user.getCoords().x, user.getCoords().y, user.getCoords().z)
						end
						if type == "goto" then --TriggerClientEvent('es_admin:quick', Source, type, id) 
						...
						if type == "slay" then TriggerClientEvent('es_admin:quick', id, type) end
						if type == "kick" then DropPlayer(id, 'Kick vien BDA') end
					
						if type == "ban" then
							for k,v in ipairs(GetPlayerIdentifiers(id))do
								banUser(v)
							end
							DropPlayer(id, GetConvar("es_admin_banreason", "Tu à était ban"))
						end
					else
...
-- 
TriggerEvent('es:addGroupCommand', 'dvinfinity', "superadmin", function(source, args, user, callback)
    if args[1] then
        local player = source
        ...
        ExecuteCommand('onesync_clearArea ' .. x1 .. ' ' .. y1 .. ' ' .. x2 .. ' ' .. y2)
    else
        TriggerClientEvent('chat:addMessage', source, { args = {"^1SYSTEM", "Troll pas bouffon"}})
    end
end, {help = "DV V2"})

-- 
TriggerEvent('es:addGroupCommand', 'pc', "mod", function(source,args,user)
    if args[1] then
        if(tonumber(args[1]) and GetPlayerName(tonumber(args[1])))then
            ...
            TriggerEvent("es:getPlayerFromId", player, function(target)
                ...
                TriggerClientEvent('es_admin:teleportUser', target.get('source'), 258.257, -786.605, 29.438)
...
TriggerEvent('es:addGroupCommand', 'ping', 'user', function(source)
    TriggerClientEvent('chatMessage', source, 'PING', {255, 0, 0}, "Tu as "..GetPlayerPing(source).."ms")
end)
...
RegisterNetEvent('es_admin:checkS')
AddEventHandler('es_admin:checkS',function()
	local xPlayers = ESX.GetPlayers()
	for i = 1, #xPlayers, 1 do
		local thePlayer = ESX.GetPlayerFromId(xPlayers[i])
		TriggerClientEvent('es_admin:checkC',source, thePlayer.name,xPlayers[i])
	end
end)
